...
function (_React$Component) {
  function FieldArray() {
    ...
    return (_temp = _this = _React$Component.call.apply(_React$Component, [this].concat(args)) || this, _this.onAdd = function (item) {
      ...
      _this.sendErrors(function (errors, name) {
        return unshift(errors, name, index);
      });
      ...
      var newValue = value == null ? [] : value.concat();
      !(fromIndex !== -1) ? process.env.NODE_ENV !== "production" ? invariant(false, '`onMove` must be called with an item in the array') : invariant(false) : void 0;
      ...
      _this.sendErrors(function (errors, name) {
        return move(errors, name, fromIndex, toIndex);
      });
      ...
      _this.sendErrors(function (errors, name) {
        return shift(errors, name, index);
      });
  ...
  _proto.onItemError = function onItemError(name, errors) {
    this.sendErrors(function (fieldErrors) {
      return _extends({}, remove(fieldErrors, name), errors);
    });
  };
  ...
  _proto.render = function render() {
    ...
    return React.createElement(Field, _extends({}, fieldProps, {
      noResolveType: true,
      __source: {
      ...
      return typeof children === 'function' ? children(nextProps) : React.cloneElement(children, nextProps);
    });
  };
